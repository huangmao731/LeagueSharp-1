...
namespace spredtest
{
    public class Blitzcrank : BaseChamp
    {
        ...
        public override void SetSpells()
        {
            ...
            Spells[Q].SetSkillshot(0.22f, 70f, 1800f, true, SkillshotType.SkillshotLine);
            ...
            if (!chase)
            {
                if (Spells[Q].IsReady() && Config.Item("CUSEQ").GetValue<bool>())
                {
                    ...
                    if (t != null && (!t.HasBuffOfType(BuffType.SpellImmunity) && !t.HasBuffOfType(BuffType.SpellShield)) && t.IsValidTarget(Spells[Q].Range - 30))
                    {
                        if (Config.Item("nograb" + t.ChampionName).GetValue<bool>())
                            return;
                        Spells[Q].SPredictionCast(t, ShineCommon.Utility.HitchanceArray[Config.Item("CQHITCHANCE").GetValue<StringList>().SelectedIndex]);
                    }
                }
        ...
        public void Harass()
        {
            ...
            if (Spells[Q].IsReady() && Config.Item("HUSEQ").GetValue<bool>())
            {
                ...
                if (target != null)
                    Spells[Q].SPredictionCast(target, ShineCommon.Utility.HitchanceArray[Config.Item("HQHITCHANCE").GetValue<StringList>().SelectedIndex]);
            }
        }
